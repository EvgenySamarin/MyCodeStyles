...
1. PREFIX - отражает тип задачи и его приоритет **TODO** - необходимо сделать до релиза (технический долг) (по умолчанию задаётся синим цветом подсветки); **BACKLOG** - неприоритетная работа, которая может быть выполнена по наличию свободного времени (улучшение читабельности кода, refactoring legacy, внедрение новой технологии, устранение мелких недостатков программы) (по умолчанию задаётся зелёным цветом подсветки); **FIXME** - высоко приоритетная работа, направленная на устранение багов и критичных уязвимостей, должна выполняться в первую очередь, постоянно на контроле (по умолчанию задаётся красным цветом подсветки); **CLEARME** - отображает вынужденную "грязь" в коде, которую следует подчистить перед релизом (заглушки, тестовые данных и т.п.) (по умолчанию задаётся желтым цветом подсветки).
